...
def extract_rois_from_heatmap(
   ...
   cy1, cx1 = _peak_loc_heatmap(heatmap)
   if heatmap[cy1, cx1] < heatmap_threshold:
      # fallback: center
      cy1, cx1 = H // 2, W // 2
   y0_a, x0_a, y1_a, x1_a = _square_box_around(cy1, cx1, carpal_margin, H, W)
   ...
   heatmap2 = _mask_disk(heatmap, cy1, cx1, radius_frac=meta_mask_radius)
...
def _mask_disk(
   ...
   radius = radius_frac * tf.cast(tf.minimum(H, W), tf.float32)
   dist = tf.sqrt(
      tf.square(tf.cast(yy - cy, tf.float32)) +
      tf.square(tf.cast(xx - cx, tf.float32))
   )
   mask = tf.cast(dist > radius, heatmap.dtype)
   return heatmap * mask
